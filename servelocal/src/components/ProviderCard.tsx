  ...
  return (
    <Link href={`/pro/${provider.slug}`} className="surface-card-hover group block">
      ...
      <div className="mt-3 flex items-start justify-between gap-3">
        <div>
          <h3 className="font-semibold text-brand-950 group-hover:text-teal-600">{provider.display_name}</h3>
      ...
      {provider.bio && (
        <p className="mt-3 line-clamp-2 text-sm leading-relaxed text-slate-500">{provider.bio}</p>
      )}
