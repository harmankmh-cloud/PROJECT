  ...
  return (
    <footer className="border-t border-slate-200/70 bg-white/50 py-12">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 sm:grid-cols-2 sm:px-8 lg:grid-cols-4">
        <div>
          <p className="font-display text-lg text-brand-950">{SERVE_LOCAL.name}</p>
          <p className="mt-2 text-sm text-slate-500">
            {SERVE_LOCAL.region}, Canada · Verified local trades directory
          </p>
        </div>
        <div>
      ...
      <p className="mt-10 text-center text-xs text-slate-400">
        © {new Date().getFullYear()} {SERVE_LOCAL.name} · Hire direct · Verify licence & insurance before work begins
      </p>
    </footer>
  );
}
