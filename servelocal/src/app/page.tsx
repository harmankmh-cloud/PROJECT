  ...
  return (
    <main className="app-bg min-h-screen">
      ...
      <section className="hero-light px-4 pb-16 pt-12 sm:px-8 sm:pb-24 sm:pt-16">
        <div className="hero-glow -left-20 top-0 h-72 w-72 bg-accent-500/20" />
        <div className="hero-glow -right-10 top-20 h-64 w-64 bg-violet-500/15" />
        <div className="relative mx-auto max-w-4xl text-center">
          <div className="mx-auto inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-white px-4 py-1.5 text-xs font-medium text-zinc-600 shadow-sm">
            <span className="h-1.5 w-1.5 rounded-full bg-success-500" />
            Fraser Valley & Metro Vancouver
          </div>
          <h1 className="font-display mt-6 text-4xl font-extrabold leading-[1.1] tracking-tight text-zinc-900 sm:text-5xl lg:text-6xl">
          ...
          <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            <Link href="/request" className="btn-primary px-8 py-3.5 text-base">
              Get free quotes
            </Link>
          ...
          <div className="mx-auto mt-12 grid max-w-2xl grid-cols-3 gap-3 sm:gap-4">
            <div className="stat-hero">
              <p className="font-display text-2xl font-bold text-zinc-900">{stats.providers || "50+"}</p>
              <p className="text-xs text-zinc-500">Local pros</p>
            </div>
            <div className="stat-hero">
              <p className="font-display text-2xl font-bold text-zinc-900">{stats.verified || "—"}</p>
              <p className="text-xs text-zinc-500">Verified</p>
            </div>
            <div className="stat-hero">
              <p className="font-display text-2xl font-bold text-zinc-900">{stats.reviews || "—"}</p>
        ...
        <div className="mx-auto grid max-w-6xl gap-8 px-4 sm:grid-cols-2 lg:grid-cols-4 sm:px-8">
          {TRUST_BADGES.map((b) => (
            <div key={b.label} className="flex gap-4">
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-accent-500/10 text-lg">
                {b.icon}
              </span>
              <div>
                <p className="font-semibold text-zinc-900">{b.label}</p>
                <p className="mt-0.5 text-sm leading-relaxed text-zinc-500">{b.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-8">
        ...
        <h2 className="font-display mt-2 text-3xl font-bold tracking-tight text-zinc-900">
      ...
      <section className="mx-auto max-w-6xl px-4 py-8 sm:px-8">
        <div className="flex items-end justify-between gap-4">
          <h2 className="font-display text-2xl font-bold tracking-tight text-zinc-900">Browse by city</h2>
          <Link href="/search" className="text-sm font-semibold text-accent-600 hover:underline">
            Search all →
          </Link>
        </div>
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {TRADE_CITIES.map((city) => (
            <Link key={city.slug} href={`/${city.slug}`} className="surface-card-hover">
              <p className="font-semibold text-zinc-900">{city.name}</p>
              <p className="text-xs text-zinc-500">{city.region}</p>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="border-t border-zinc-200 bg-white py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-8">
          <div className="flex items-end justify-between gap-4">
            ...
            <Link href="/guides" className="text-sm font-semibold text-accent-600 hover:underline">
              All guides →
            </Link>
          </div>
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {categories.map((cat) => (
              <Link
                ...
                className="surface-card flex items-center gap-3 p-4 transition hover:border-accent-500/30"
              >
                <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-zinc-100 text-xl">
                  {cat.icon}
                </span>
                <div>
                  <span className="font-medium text-zinc-900">{cat.name}</span>
                  <p className="text-xs text-zinc-500">BC price guide</p>
                </div>
              </Link>
            ))}
          </div>
      ...
      <section className="hero-dark px-4 py-16 sm:px-8">
        <div className="hero-glow left-1/4 top-0 h-64 w-64 bg-accent-500/25" />
