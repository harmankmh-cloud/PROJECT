...
export function SupportForm({
  ...
  if (sent) {
    return (
      <div className="surface-card p-8 text-center">
        ...
        <h3 className="font-display mt-3 text-xl text-brand-950">Message sent</h3>
        <p className="mt-2 text-sm text-slate-600">
          The {BRAND.name} team got your note. We usually reply by email within 1–2 business days.
        </p>
  ...
  return (
    <form onSubmit={handleSubmit} className={compact ? "space-y-4" : "surface-card space-y-5 p-6 sm:p-8"}>
      {!compact && (
        <div>
          <h2 className="font-display text-xl text-brand-950">Send us a message</h2>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block space-y-2 text-sm sm:col-span-2">
          <span className="font-semibold text-brand-950">Your email</span>
          <input
            type="email"
            value={email}
        ...
        <label className="block space-y-2 text-sm">
          <span className="font-semibold text-brand-950">Name (optional)</span>
          <input value={name} onChange={(e) => setName(e.target.value)} className="input-field" placeholder="Harman" />
        </label>
        <label className="block space-y-2 text-sm">
          <span className="font-semibold text-brand-950">Business (optional)</span>
          <input
            value={businessName}
      ...
      <label className="block space-y-2 text-sm">
        <span className="font-semibold text-brand-950">Topic</span>
        <select
          value={category}
      ...
      <label className="block space-y-2 text-sm">
        <span className="font-semibold text-brand-950">Message</span>
        <textarea
          value={message}
