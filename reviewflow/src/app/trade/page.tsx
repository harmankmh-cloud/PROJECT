...
import { TRADE_CITIES, TRADE_LOCAL } from "@/lib/tradelocal/constants";
import { getApprovedProviders, getServiceCategories } from "@/lib/tradelocal/data";

export default async function TradeLocalHomePage() {
    ...
    <main className="mesh-bg min-h-screen">
      <header className="site-header">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-8">
          <Link href="/trade" className="font-display text-xl text-brand-950">
            {TRADE_LOCAL.name}
          </Link>
          <nav className="flex gap-2 sm:gap-3">
            <Link href="/" className="hidden text-sm text-slate-600 hover:text-brand-950 sm:inline">
              RateLocal
            </Link>
            <Link href="/trade/join" className="btn-ghost px-4 py-2 text-sm">
              List your business
            </Link>
            <Link href="/trade/request" className="btn-gold px-4 py-2 text-sm">
              Post a job
            </Link>
          </nav>
        </div>
      ...
      <section className="hero-dark relative overflow-hidden px-4 py-16 sm:px-8 sm:py-24">
        ...
        <div className="relative mx-auto max-w-3xl text-center text-white">
          ...
          <div className="mt-10 flex flex-col justify-center gap-3 sm:flex-row">
            <Link href="/trade/request" className="btn-gold px-8 py-4 text-base">
              I need someone
            </Link>
            <Link href="/trade/join" className="inline-flex items-center justify-center rounded-xl border border-white/25 px-8 py-4 text-base font-semibold text-white hover:bg-white/10">
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-8">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {TRADE_CITIES.map((city) => (
            <Link key={city.slug} href={`/trade/${city.slug}`} className="surface-card-hover p-5">
        ...
        <section className="mx-auto max-w-6xl px-4 py-16 sm:px-8">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {featured.slice(0, 6).map((p) => (
              <Link key={p.id} href={`/trade/pro/${p.slug}`} className="surface-card-hover p-5">
      ...
      <footer className="border-t border-slate-200/70 py-10 text-center text-sm text-slate-500">
        ...
        <p className="mt-4">{TRADE_LOCAL.name} · Part of the RateLocal family · BC, Canada</p>
      </footer>
    </main>
  );
}
