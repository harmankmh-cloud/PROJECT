...
const bodySchema = z.object({
  ...
  /** Mark these emails as already sent (won't email again). */
...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const raw of body.leads) {
      ...
      if (!email) {
        const { error } = await admin.from("va_outreach_leads").insert(row);
      ...
      if (existing) {
        ...
        const { error } = await admin.from("va_outreach_leads").update(row).eq("id", existing.id);
