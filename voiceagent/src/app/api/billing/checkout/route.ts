...
export async function POST(request: NextRequest) {
  ...
  if (!priceId || !priceId.startsWith("price_")) {
    return NextResponse.json(
      {
        error: `No Stripe price found for ${plan}. Create "Intellivo ${plan}" ($${plan === "starter" ? 99 : plan === "pro" ? 499 : 2000}/mo) in Stripe, or set STRIPE_PRICE_${plan.toUpperCase()}_MONTHLY.`,
      },
      { status: 400 }
    );
  }
