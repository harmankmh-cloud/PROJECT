...
export function GoogleAuthButton({
  ...
  async function handleGoogle() {
    if (mode === "signup" && !termsAccepted) {
      setTermsError("You must accept the terms to continue with Google.");
      return;
    }
    setTermsError("");
    setLoading(true);
    ...
    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=${mode === "signup" ? "/onboarding?welcome=1" : "/dashboard"}`,
      },
    });
  }
