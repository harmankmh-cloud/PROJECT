...
export async function UsageMeterBanner() {
  ...
  const events = await fetchPeriodUsageEvents(admin, org.id, currentBillingPeriodStart());
  const { totalMinutes, overageMinutes, percentUsed } = summarizeUsage(events, included);
